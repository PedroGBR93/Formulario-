document.addEventListener('DOMContentLoaded', () => {
    ...
    const calculosAutomaticos = [{
        plan: 'plan_dia_accesorio_gen',
        prod: 'producido_accesorio_gen',
        label: 'label-accgen-diferencia',
        output: 'diferencia_accesorio_gen',
        baseName: 'Accesorios'
    }, {
        plan: 'plan_dia_plumones',
        prod: 'producido_plumones',
        label: 'label-accplum-diferencia',
        output: 'diferencia_plumones',
        baseName: 'Plumones'
    }, {
        plan: 'plan_dia_almohadas',
        prod: 'producido_almohadas',
        label: 'label-accalm-diferencia',
        output: 'diferencia_almohadas',
        baseName: 'Almohadas'
    }, {
        plan: 'bases_plan_total',
        prod: 'bases_producido',
        label: 'label-bases-diferencia',
        output: 'diferencia_bases',
        baseName: 'Bases'
    }, {
        plan: 'muebles_sofas_plan',
        prod: 'muebles_sofas_producido',
        label: 'label-muebsofa-diferencia',
        output: 'diferencia_sofas',
        baseName: 'Sofás'
    }, {
        plan: 'muebles_reclinables_plan',
        prod: 'muebles_reclinables_producido',
        label: 'label-muebrecl-diferencia',
        output: 'diferencia_reclinables',
        baseName: 'Reclinables'
    }, {
        plan: 'muebles_respaldos_plan',
        prod: 'muebles_respaldos_producido',
        label: 'label-muebresp-diferencia',
        output: 'diferencia_respaldos',
        baseName: 'Respaldos'
    }];
        ...
        for (const id in APP_CONFIG.textMappings) {
            ...
            if (element) {
                ...
                if (id === "label-cumplimiento" && cumplimientoValorDisplay) {
                    ...
                    if (nodoTexto) {
                        nodoTexto.nodeValue = textoCompleto + " ";
                    } else {
                        element.innerHTML = `${textoCompleto} <span id="cumplimiento_valor_display">${valorActual}</span>%`;
                    }
                } else {
    ...
    function updateFormVisibility() {
        ...
        if (conditionalElements.observaciones) {
            conditionalElements.observaciones.style.display = 'block';
        }
    ...
    function crearHtmlSuceso(idCounter, displayIndex) {
        ...
        return `<div class="dynamic-row-entry" id="suceso_entry_${idCounter}">
                    ...
                    <div class="form-group">
                        <label for="incidente_${idCounter}">Incidente:</label>
                        <input type="text" id="incidente_${idCounter}" name="sucesos[${idCounter}][incidente]" placeholder="Descripción del incidente">
                    </div>
                    <div class="form-group">
                        <label for="tipo_incidente_${idCounter}">Tipo de incidente:</label>
                        <select id="tipo_incidente_${idCounter}" name="sucesos[${idCounter}][tipo_incidente]">
                            <option value="">Seleccione</option>${opcionesTipoIncidente}
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="area_responsable_${idCounter}">Área Responsable:</label>
                        <select id="area_responsable_${idCounter}" name="sucesos[${idCounter}][area_responsable]">
                            <option value="">Seleccione</option>${opcionesAreaResponsable}
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="tiempo_afectacion_${idCounter}">Tiempo de afectación (Min):</label>
                        <input type="number" id="tiempo_afectacion_${idCounter}" name="sucesos[${idCounter}][tiempo_afectacion]" min="0" placeholder="Ej: 60">
                    </div>
                    <div class="form-group">
                        <label for="unidades_${idCounter}">${unidadesLabelText}</label>
                        <input type="text" id="unidades_${idCounter}" name="sucesos[${idCounter}][unidades]" placeholder="Ej: 10 o Texto">
                    </div>
                    <div class="form-group">
                        <label for="impacto_produccion_${idCounter}">Impacto en producción:</label>
                        <textarea id="impacto_produccion_${idCounter}" name="sucesos[${idCounter}][impacto_produccion]" rows="1" placeholder="Describa el impacto"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="gestion_tomada_${idCounter}">Gestión tomada:</label>
                        <textarea id="gestion_tomada_${idCounter}" name="sucesos[${idCounter}][gestion_tomada]" rows="1" placeholder="Describa la gestión"></textarea>
                    </div>
                    <div class="form-group checkbox-group">
                        <input type="checkbox" id="estado_resuelto_${idCounter}" name="sucesos[${idCounter}][estado_resuelto]" value="true">
                        <label for="estado_resuelto_${idCounter}" class="checkbox-label">Resuelto</label>
                    </div>
    ...
    function crearHtmlAusentismo(idCounter, displayIndex) {
        return `<div class="dynamic-row-entry" id="ausentismo_entry_${idCounter}">
                    <h3 class="dynamic-row-title">Reporte Ausentismo ${displayIndex}</h3>
                    <div class="form-group">
                        <label for="puesto_trabajo_${idCounter}">Puesto De Trabajo:</label>
                        <input type="text" id="puesto_trabajo_${idCounter}" name="ausentismos[${idCounter}][puesto_trabajo]" placeholder="Ej: Operador de máquina X">
                    </div>
                    <div class="form-group">
                        <label for="Motivo Ausentismo_${idCounter}">Motivo:</label>
                        <select id="motivo_ausentismo_${idCounter}" name="ausentismos[${idCounter}][motivo]">
                            <option value="">Seleccione</option>
                            <option value="Licencia Medica">Licencia Médica</option>
                            <option value="Permiso Otorgado">Permiso Otorgado</option>
                            <option value="Ausencia Sin Justificar">Ausencia Sin Justifica</option>
                            <option value="Por Cumpleaños">Por Cumpleaños</option>
                            <option value="Información Por Ratificar">Información Por Ratificar</option>
                            <option value="Otro motivo">Otro Motivo</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="cantidad_personal_${idCounter}">Cantidad De Personal:</label>
                        <input type="number" id="cantidad_personal_${idCounter}" name="ausentismos[${idCounter}][cantidad_personal]" min="1" value="1" placeholder="Ej: 1">
                    </div>
                    <div class="form-group">
                        <label for="gestion_ausentismo_${idCounter}">Gestión:</label>
                        <textarea id="gestion_ausentismo_${idCounter}" name="ausentismos[${idCounter}][gestion]" rows="1" placeholder="Describa la gestión realizada"></textarea>
                    </div>
                    <button type="button" class="btn-remove-row icon-button" onclick="eliminarFila('ausentismo_entry_${idCounter}', 'ausentismo-container', 'Reporte Ausentismo')">
                        <span class="trash-icon-placeholder">&#128465;</span> Eliminar Reporte ${displayIndex}
                    </button>
                    <hr class="dynamic-row-divider">
                </div>`;
    }
        ...
        return `<div class="dynamic-row-entry" id="hora_extra_entry_${idCounter}">
                    ...
                    <div class="horas-extra-fields">
                        ...
                        <div class="form-group">
                            ...
                            <select id="subarea_${idCounter}" name="horas_extra[${idCounter}][subarea]" class="subarea-select">
                                <option value="">Seleccione</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="motivo_he_${idCounter}">Motivo:</label>
                            <select id="motivo_he_${idCounter}" name="horas_extra[${idCounter}][motivo]">
                                <option value="">Seleccione</option>
    ...
    function actualizarSubAreasEnFilas() {
        ...
        todosLosSelects.forEach(select => {
            ...
            if (subAreas.includes(valorActual)) {
    ...
    function recolectarDatosDelFormulario() {
        ...
        datos.fecha = (fechaInput && fechaInput.value) ? new Date(fechaInput.value).toLocaleString('es-CL', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
            hour: '2-digit',
        ...
        if (conditionalElements.accesoriosPlan.style.display !== 'none') {
            datos.accesoriosPlan = {
                general: {
                    plan_dia: formData.get('accesorios_plan[general][plan_dia]'),
                    producido: formData.get('accesorios_plan[general][producido]'),
                    diferencia: getDiferenciaData('accesorios_plan[general][plan_dia]', 'accesorios_plan[general][producido]', 'label-accgen-diferencia')
                },
                plumones: {
                    plan_dia: formData.get('accesorios_plan[plumones][plan_dia]'),
                    producido: formData.get('accesorios_plan[plumones][producido]'),
                    diferencia: getDiferenciaData('accesorios_plan[plumones][plan_dia]', 'accesorios_plan[plumones][producido]', 'label-accplum-diferencia')
                },
                almohadas: {
                    plan_dia: formData.get('accesorios_plan[almohadas][plan_dia]'),
                    producido: formData.get('accesorios_plan[almohadas][producido]'),
                    diferencia: getDiferenciaData('accesorios_plan[almohadas][plan_dia]', 'accesorios_plan[almohadas][producido]', 'label-accalm-diferencia')
                }
            };
        }
        if (conditionalElements.basesPlan.style.display !== 'none') {
            datos.basesPlan = {
                planTotal: formData.get('bases_plan[plan_total]'),
                producido: formData.get('bases_plan[producido]'),
                diferencia: getDiferenciaData('bases_plan[plan_total]', 'bases_plan[producido]', 'label-bases-diferencia')
            };
        }
        if (conditionalElements.mueblesPlan.style.display !== 'none') {
            datos.mueblesPlan = {
                sofas: {
                    plan_dia: formData.get('muebles_plan[sofas][plan_dia]'),
                    producido: formData.get('muebles_plan[sofas][producido]'),
                    diferencia: getDiferenciaData('muebles_plan[sofas][plan_dia]', 'muebles_plan[sofas][producido]', 'label-muebsofa-diferencia')
                },
                reclinables: {
                    plan_dia: formData.get('muebles_plan[reclinables][plan_dia]'),
                    producido: formData.get('muebles_plan[reclinables][producido]'),
                    diferencia: getDiferenciaData('muebles_plan[reclinables][plan_dia]', 'muebles_plan[reclinables][producido]', 'label-muebrecl-diferencia')
                },
                respaldosOtros: {
                    plan_dia: formData.get('muebles_plan[respaldos_otros][plan_dia]'),
                    producido: formData.get('muebles_plan[respaldos_otros][producido]'),
                    diferencia: getDiferenciaData('muebles_plan[respaldos_otros][plan_dia]', 'muebles_plan[respaldos_otros][producido]', 'label-muebresp-diferencia')
                }
            };
        }

        if (conditionalElements.fibraScrap.style.display !== 'none') {
            datos.ingresoScrapFibra = formData.get('ingreso_scrap_fibra');
        }
        if (conditionalElements.espumaScrap.style.display !== 'none') {
            datos.salidaScrapEspuma = formData.get('salida_scrap_espuma');
        }
        if (conditionalElements.colchonesProd.style.display !== 'none') {
            datos.colchonesProd = {
                planDia: formData.get('colchones_prod[plan_dia]'),
                producido: formData.get('colchones_prod[producido]'),
                porVulcanizar: formData.get('colchones_prod[por_vulcanizar]'),
                porCerrar: formData.get('colchones_prod[por_cerrar]')
            };
        }
        if (conditionalElements.datosRelevante.style.display !== 'none') {
            datos.datosRelevanteColchones = {
                personalCerrado: formData.get('datos_relevante[colchones][personal_cerrado]'),
                colchonesReparados: formData.get('datos_relevante[colchones][reparados]'),
                esperaReparacion: formData.get('datos_relevante[colchones][espera]')
            };
        }
        ...
        sucesosContainer.querySelectorAll('.dynamic-row-entry').forEach(entry => {
            const id = entry.id.split('_').pop();
            datos.sucesos.push({
                incidente: formData.get(`sucesos[${id}][incidente]`),
                tipoIncidente: formData.get(`sucesos[${id}][tipo_incidente]`),
                areaResponsable: formData.get(`sucesos[${id}][area_responsable]`),
                tiempoAfectacion: formData.get(`sucesos[${id}][tiempo_afectacion]`),
                unidades: formData.get(`sucesos[${id}][unidades]`),
                impactoProduccion: formData.get(`sucesos[${id}][impacto_produccion]`),
                gestionTomada: formData.get(`sucesos[${id}][gestion_tomada]`),
        ...
        datos.ausentismos = [];
        ausentismoContainer.querySelectorAll('.dynamic-row-entry').forEach(entry => {
            const id = entry.id.split('_').pop();
            datos.ausentismos.push({
                puestoTrabajo: formData.get(`ausentismos[${id}][puesto_trabajo]`),
                motivo: formData.get(`ausentismos[${id}][motivo]`),
                cantidadPersonal: formData.get(`ausentismos[${id}][cantidad_personal]`),
                gestion: formData.get(`ausentismos[${id}][gestion]`)
            });
        });
        datos.horasExtra = [];
        if (horasExtraContainer) {
            horasExtraContainer.querySelectorAll('.dynamic-row-entry').forEach(entry => {
                const id = entry.id.split('_').pop();
                datos.horasExtra.push({
                    numeroHoras: formData.get(`horas_extra[${id}][numero_horas]`),
                    cantidadPersonal: formData.get(`horas_extra[${id}][cantidad_personal]`),
                    subarea: formData.get(`horas_extra[${id}][subarea]`),
                    motivo: formData.get(`horas_extra[${id}][motivo]`)
                });
            });
        }
    ...
    if (areaSelect) {
        areaSelect.addEventListener('change', updateFormVisibility);
    }
    if (btnAnadirSuceso) {
        btnAnadirSuceso.addEventListener('click', anadirNuevaFilaSuceso);
    }
    if (btnAnadirAusentismo) {
        btnAnadirAusentismo.addEventListener('click', anadirNuevaFilaAusentismo);
    }
    if (btnAnadirHoraExtra) {
        btnAnadirHoraExtra.addEventListener('click', anadirNuevaFilaHoraExtra);
    }
    if (observacionesTextarea) {
        setupAutoResizeForTextarea(observacionesTextarea);
        ...
        form.addEventListener('submit', function(event) {
            ...
            try {
                if (!areaSelect || !areaSelect.value) {
                    alert("Por favor, seleccione un Área antes de enviar el reporte.");
                    return;
                }
